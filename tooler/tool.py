...
class Node:
    ...
    def disconnect(self):
        """既存接続を解除"""
        lines_to_remove = []  # 削除対象の接続線を一時保存

        for line in self.editor.lines:
            if line[1] == self:  # 入力円側の接続を解除
                self.editor.canvas.delete(line[2])  # キャンバスから線を削除
                lines_to_remove.append(line)       # 削除対象として保存

                # 接続元ノードからも接続解除
                if line[0] in self.connections:
                    self.connections.remove(line[0])

        # `self.editor.lines` から該当する接続線を一括削除
    ...
    def connect_to(self, other_node):
        ...
        x1, y1 = self.x + 120, self.y + 30
        x2, y2 = other_node.x, other_node.y + 30
