...
class CustomMenuOnCanvas:
    def __init__(self, canvas, items, width=180, bg_color="#f0f0f0", highlight_color="#c0c0c0", border_color="#a0a0a0", transparent_color="#00ff00", radius=10):
        self.canvas = canvas  # rootをcanvasに変更
        self.items = items
        self.width = width
        ...
        # キャンバス上の右クリックでメニューを表示
        self.canvas.bind("<Button-3>", self.show_menu)

        # キャンバスが属するウィンドウが最小化されたときにメニューを閉じる
        self.canvas.winfo_toplevel().bind("<Unmap>", self.on_canvas_minimize)
    ...
    def show_menu(self, event):
        ...
        # メインメニューの作成と最前面設定
        self.menu = Toplevel(self.canvas)
    ...
    def hide_submenu(self):
        ...
        if self.submenu and not self.submenu_active:
            self.canvas.after(300, self._destroy_submenu)
    ...
    def on_canvas_minimize(self, event):
        if self.menu:
            self.menu.destroy()
        if self.submenu:
            self.submenu.destroy()
...
custom_menu = CustomMenuOnCanvas(canvas, items)
