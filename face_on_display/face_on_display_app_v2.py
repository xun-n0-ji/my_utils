...
class FaceOnDisplayApp:
    ...
    def __init__(self):
        ...
        # 画像のサイズを取得
        image_height = int(self.cap.get(cv2.CAP_PROP_FRAME_WIDTH))
        image_width = int(self.cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
        ...
        if window_aspect_ratio > image_aspect_ratio:
            ...
            self.display_width = int(self.window_height * image_aspect_ratio)
        else:
            # 画像のアスペクト比がウィンドウのアスペクト比より大きい場合、ウィンドウの幅を画像の幅に合わせる
            self.display_width = self.window_width
            self.display_height = int(self.window_width / image_aspect_ratio)
    ...
    def update_frame(self):
        ...
        if ret:
            ...
            # キャンバス上に画像を描画
            self.canvas.create_image((self.window_width - self.display_width) // 2, (self.window_height - self.display_height) // 2, anchor="nw", image=imgtk)
